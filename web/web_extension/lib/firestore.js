...
export async function setDocMerge(docPath, data, serverTimestampFields = []) {
  ...
  if (fieldPaths.length === 0 && fieldTransforms.length > 0) {
    // Transform-only write (doesn't touch other fields).
    writes = [{ transform: { document: name, fieldTransforms } }];
  } else {
    // Update (merge by providing updateMask for the provided fields).
    const w = {
      update: { name, fields },
  ...
  const res = await authedFetch(commitUrl, { method: "POST", body: JSON.stringify({ writes }) });
  const json = await res.json().catch(() => null);
  if (!res.ok) throw new Error(json?.error?.message || `HTTP ${res.status}`);
  return true;
}
...
export async function addDoc(collectionPath, data, serverTimestampFields = []) {
  // We create a random ID client-side for consistent transforms.
