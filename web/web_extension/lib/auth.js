...
export async function getValidIdToken() {
  ...
  // If JWT exp is available, prefer it
  const exp = sess.exp;
  if (exp && exp - nowSec() > 90) return sess.idToken;

  // fallback to expiresAt
  if (sess.expiresAt && sess.expiresAt - Date.now() > 90_000) return sess.idToken;
  ...
  const data = await res.json().catch(() => null);
  if (!res.ok) throw new Error(data?.error?.message || `Token refresh failed (HTTP ${res.status})`);
