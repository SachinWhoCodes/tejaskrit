...
async function loadConfig() {
  ...
  // Merge saved settings but ALWAYS force backendBaseUrl to the production Candidate app.
  // This keeps resume generation + tracker link consistent for all users.
  return { ...DEFAULT_CONFIG, ...(saved || {}), backendBaseUrl: DEFAULT_CONFIG.backendBaseUrl };
}
...
export async function upsertPrivateJob(jobId, jobInfo) {
  ...
  const ts = existing ? ["updatedAt", "lastSeenAt"] : ["createdAt", "updatedAt", "postedAt", "lastSeenAt"];
...
export async function upsertApplicationStatus(args) {
  ...
  const [user, existing] = await Promise.all([getDoc(`users/${uid}`), getDoc(`applications/${appId}`)]);
  const instituteId = user?.instituteId ?? null;

  const keepAppliedAt = existing?.appliedAt instanceof Date ? existing.appliedAt : null;
  const appliedAt =
    args.status === "applied"
...
export async function generateTailoredLatex(jobId, matchScore, matchReasons) {
  ...
  return json; // { ok, applicationId, genId }
}
...
export async function downloadResumePdf(applicationId) {
  ...
  const token = await getValidIdToken();
  const res = await fetch(`${base}/api/resume/pdf?applicationId=${encodeURIComponent(applicationId)}`, {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${token}`,
    },
